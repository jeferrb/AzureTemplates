...
run_bench() {
  ...
  for i in `seq ${repetitions}`; do
    ...
    mpirun -np "${nprocessors}" -mca plm_rsh_args "-o StrictHostKeyChecking=no"  --hostfile hostfile "${path}${name}" | tee -a "${name}_native.log"
    date | tee -a "${name}_native.log"
  ...
  for i in `seq ${repetitions}`; do
    ...
    mpirun -np "${nprocessors}" -mca plm_rsh_args "-o StrictHostKeyChecking=no"  --hostfile hostfile singularity exec /home/username/ubuntu.img "${path}${name}" | tee -a "${name}_singularity.log"
    date | tee -a "${name}_singularity.log"
