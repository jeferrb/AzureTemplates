...
createMachines(){
    ...
    --template-file azuredeploy_clean_non_image.json --parameters vmSize="${VM_SIZE}" vmName="testMpi${1}" \
    dnsLabelPrefix="my${GROUP_NAME}dnsprefix${1}" \
    adminPassword="pass$RANDOM" scriptParameterPassMount=$2 adminPublicKey="`cat ~/.ssh/id_rsa.pub`" >> ${LOG_FILE}
}

install_dependencies(){
    echo "Installing dependencies on machine $1"
...
ssh ${SSH_ADDR} << EOF
  ...
  # for ((core=0; \$((2**core))<=${NUMBER_JOBS}; core++ )); do
    ...
    for ((simulation=1; simulation<=${NUMBER_REPETITIONS}; simulation++ )); do
        echo "Going to run on \$core the simulation \$simulation"
        mpirun --mca btl ^openib -np \$((2**\$core)) --oversubscribe --hostfile hostfile \
        flow "\$HOME/OPM/opm-data/norne/NORNE_ATW2013.DATA" output_dir=out_parallel_N\${core}_\${simulation}
    done
  done
EOF
mkdir -p ${RESULTS_DIRECTORY}
scp -r "${SSH_ADDR}:/home/username/out_parallel_N*" ${RESULTS_DIRECTORY}
